...
const Register: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error(err);
      setErrorMsg(err.message || 'An account with this email already exists.');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="min-h-screen bg-[#0b1326] text-[#dae2fd] flex items-center justify-center px-4 relative overflow-hidden">
      ...
      <div className="w-full max-w-md glass-card rounded-[2rem] p-8 border border-white/10 relative z-10 space-y-6">
        <div className="text-center space-y-2">
          ...
          <h2 className="text-xl font-bold text-white font-headline-lg">Create account</h2>
          <p className="text-xs text-slate-400">Join students globally mapping concept mind-maps.</p>
        </div>
